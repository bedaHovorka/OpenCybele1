...
|---|---|---|
| Periodic / timer activity | `TickerBehaviour` (`onTick()`) | ⚠️ **Not the mapping this repo needs.** `TickerBehaviour` is "a cyclic task that must be executed periodically" (Programmer's Guide §3.4.10, <https://jade-project.gitlab.io/docs/programmersguide.pdf>) — but **this codebase has zero repeating timers**. All four timer sites ([`INVENTORY.md`](INVENTORY.md) §6, `TMR-01`…`TMR-04`) are one-shot or **self-rearming with a freshly drawn interval**: `TMR-02` redraws an exponential delay on every fire, `TMR-03`/`TMR-04` are per-train/per-traversal one-shots, `TMR-01` is a bootstrap. The faithful shape is a one-shot wake-up that re-arms itself. **[#33](https://github.com/bedaHovorka/OpenCybele1/issues/33) landed it and narrowed the shape once more**: not `WakerBehaviour` either, because a `WakerBehaviour` deadline is **wall clock** and is fixed when the behaviour is added, so it would ignore the toolbar's runtime pace change (`Gui.java:98`) and keep arriving across a `pause()`. All four sites therefore go on `AgentClock`, whose deadlines are absolute **simulated** instants, drained by one `ClockTickerBehaviour` per agent — see [`clock-abstraction.md`](clock-abstraction.md) §3.2. So this row has **no instance at all** in this repo, which is worth more than a caveat on it: the one mapping the table calls "near 1:1" is the one that never applies. |
...
| Cybele message | `ACLMessage` + performative | ⚠️ **Decided, per channel, in [`message-ontology.md`](message-ontology.md)** ([#27](https://github.com/bedaHovorka/OpenCybele1/issues/27)). All fifteen channels are plain unicast `ACLMessage`s; the performative assignment is 4×`request`, 7×`inform`, and one each of `cfp`, `propose`, `accept-proposal`, `query-ref`. "Some redesign" understates one thing and overstates another: the payloads become named immutable records (a real change — `Station.Info` stops being a live alias), but **Cybele has no performatives at all**, so every one of the fifteen is a decision this port invents rather than a translation. |
| Directory / community | **AMS** (white pages) — applies. **DF** (yellow pages) — **does not apply to this repo.** | The AMS half is right: JADE addresses agents by `AID`, and the AMS "provides white-page and life-cycle service, maintaining a directory of agent identifiers (AID) and agent state" (Programmer's Guide §3.1 "The Agent Platform", <https://jade-project.gitlab.io/docs/programmersguide.pdf>). The DF half is wrong here: the DF "provides the default yellow page service" (same source), and **nothing in this application advertises or searches a capability** — [`INVENTORY.md`](INVENTORY.md) §4 shows all 15 channels (`CH-01`…`CH-15`) with **exactly one subscriber each**, addressed by literal name. The faithful mapping is therefore **direct AID unicast**, not DF lookup. JADE **topics** (`jade.core.messaging.TopicManagementHelper.createTopic()`/`register()`, `MessageTemplate.MatchTopic()`, with `TopicManagementService` activated on every container; available "since version 3.5" — Programmer's Guide §3.3.5) are the right tool for the *probe*, which needs to observe traffic it is not the named receiver of — not for normal delivery. The channel→`ACLMessage` design itself is [#27](https://github.com/bedaHovorka/OpenCybele1/issues/27)'s and is now specified in [`message-ontology.md`](message-ontology.md), which also decides topic granularity: **per channel constant** — fifteen topics fixed at boot, rather than one per channel *instance*, which would be 93 at boot and four more per train forever. |
| Agent creation (`Cybele.createAgent(name, className, args)`) | `ContainerController.createNewAgent(name, className, args).start()` | Near 1:1 on the call, and **neither framework orders the constructors** — but only one side of the port has to care. ⚠️ `start()` returns immediately and when each child's `setup()` runs is up to the platform; `Cybele.createAgent` is asynchronous too, and [#15](https://github.com/bedaHovorka/OpenCybele1/issues/15) measured **six distinct constructor orders in six runs** on the baseline. That is already handled *outside* both implementations: `Station` and `RoadAgent` emit their first `STATION_INFO`/`ROAD_STATE` from the constructor, and [`trace-normalizer.md`](trace-normalizer.md) §2.1's `startup-block` rule sorts the **maximal opening run** of those lines by agent name — the frozen goldens are that sorted result, not any order a platform produced. What a port must add is not an ordering but the rule's **precondition**: that run has to be unbroken, so every opener must precede the first non-constructor trace line, which is `Generator`'s first `PLAN_TRAIN` at `sim.arrival.firstFireMs`. `opencybele-strict.yaml` sets that to 200 ms at pace 8 — **25 real milliseconds** for fifteen agents to boot — and JADE's per-agent boot (a thread and an AMS registration each) is heavier than a Cybele thread-pool dispatch. [#33](https://github.com/bedaHovorka/OpenCybele1/issues/33) therefore arms the generator from the **fifteenth opener** rather than from `setup()`, using messages the ontology already has (CH-10/CH-11 are addressed to `Main` anyway), and adds a watchdog that names a child that never booted rather than arming past it. A readiness handshake was rejected: it buys a total order the normalizer immediately sorts away, and costs a sixteenth channel. |
