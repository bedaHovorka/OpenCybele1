...
>
> **Revised for [#72](https://github.com/bedaHovorka/OpenCybele1/issues/72):** DEF-07's *class* is
> unchanged and correct — (a), deterministic, the port must reproduce it — but §3.1's claim that
> the ordering is “trace-visible” was wrong, and both its **How it is pinned** and **If it shows
> in a diff** cells are rewritten. The ordering survives the projection on a *queued* hop only.
> Measurement, and the full sweep of raw-line assertions: `docs/raw-assertion-audit.md`.
...
| **DEF-07** | `Train.java:82-84` | `leaveObject(position)` is sent from `entered`, i.e. **after** the new object already incremented its occupancy in `Station.enter` (`Station.java:93`). For the width of the overlap both objects count the train, and admission decisions run against the inflated number. | **Measured [this issue]** in a `sim.trace.enabled=true` run: `ENTER_REPLY` from the new object and `LEAVE` to the old object carry the **same tick**, `ENTER_REPLY` first. The *mechanism* is deterministic — it happens on every hop. | **CORRECTED by [#72](https://github.com/bedaHovorka/OpenCybele1/issues/72); the (a) classification stands, the observable does not.** The *send* order is deterministic. It is **not trace-visible on an uncontended hop**: the reply comes back in the same tick, so #21's `burst-order` rule sorts the two lines together — `parity-tests/golden/opencybele-lifecycle.txt` in fact lists a train's destructor `LEAVE` **before** its hop `LEAVE`, the reverse of the emission order — and reading the order off the RAW stream instead reads the probe's *handling* order, which inverts in 2 captures of 60 (`docs/raw-assertion-audit.md` §2). Pin it where the projection can carry it: on a **queued** hop the wait defers the `LEAVE` for the old object into a LATER burst than the `ENTER` on the new one, and the golden holds that positionally — `opencybele-congestion`, asserted by `OpenCybeleCongestionIT.assertDef07SurvivesTheProjection`. Everywhere else pin the **pairing**, which no ordering can perturb: route reconstructed from `ENTER.position`, `ENTER_REPLY.next` agreeing with it, one `LEAVE` per visited object (`OpenCybeleLifecycleIT`). **The first hop is still the exception for any checker:** `leaveObject` is a no-op while `position == null` (`Train.java:110-114`), so a train's first `ENTER_REPLY` has **no paired `LEAVE`** — verified in the trace, `vl0`'s route gives 9 objects and 9 `LEAVE`s, not 10. A rule written literally from the pre-#72 form of this cell false-positives on **every** train. Do **not** pin the resulting `occupied` number — that half is **(b)**, see DEF-13. | A `LEAVE` for the old object that moves OUT of the queued burst in `opencybele-congestion` is a port bug — that is the one place the ordering is contractual. An `ENTER_REPLY`/`LEAVE` pair in the other order **inside one burst** is **not**: within a burst the golden carries the sort, not the emission order, and the raw stream carries the probe's handling order. A different `occupied` count is not a port bug either — it is projected. |
...
| **agent init order** | `Cybele.createAgent` (async) | The order in which agent **constructors** actually run — hence the order of the opening `STATION_INFO`/`ROAD_STATE` block. | **Measured** (#15, `seeded-rng.md:79-87`): "**six distinct orders in six runs**". #19 pinned only the order in which create *requests* are issued (`iteration-order.md`); `trace-format.md:111-112` independently records that "the eight opening `STATION_INFO` lines are emitted in a different order every run". | **Requirement levied on #21 by this document:** the normalizer must **sort the opening state block by agent name**. ~~This is *not yet done*~~ — **done**, as `OrderingRule.STARTUP_BLOCK` (`trace-normalizer.md` §2.1), and the port depends on it: #33 added no readiness handshake precisely because a handshake would buy a total order this rule immediately sorts away. What a port must supply instead is the rule's **precondition** — the opening run must be unbroken, because the rule stops at the first non-constructor line. (Status refreshed by [#41](https://github.com/bedaHovorka/OpenCybele1/issues/41).) | A different startup order is never a port bug. If #21 has not sorted it, the diff is a **harness** bug. |
...
| **DEF-24** | `RailwayMainAgent.java:287-288` | `TableModel.update` builds `new ArrayList<>(trainStates.entrySet())` **off-lock** while agent threads mutate the `synchronizedMap` — `Collections.synchronizedMap` guarantees nothing for iteration without manual synchronisation — then caches the live `Map.Entry` views, whose behaviour after `remove` is undefined. | **Inferred** (#9) from the code; **never observed** — 0 `Exception in thread "` across **146** measurement runs. | **Cannot be excluded, and headless does not help** — see §4.5. Reclassified from (b) to (c) in this revision round: the earlier handling rested on `INVENTORY`'s "Swing EDT", which is wrong. | **Do not file.** A `ConcurrentModificationException` from `TableModel.update` is out-of-contract — **discard the recording** (§8.2). ~~JADE ports have no Swing `TableModel` at all, so this can never legitimately appear as a port-side diff.~~ **The reason was wrong and is corrected by [#41](https://github.com/bedaHovorka/OpenCybele1/issues/41): the JADE port DOES have a `TableModel`** — #35 ported the Swing view rather than dropping it. The conclusion survives by a different route: the *first* half of the race is structurally gone, because under Cybele several agent threads mutated the map (EVT-04) and under JADE every mutation and every `railwayChanged()` happens on the hub agent's single thread; the *second* half is not gone, because the Swing EDT still reads the model. See `RailwayMainAgent`'s class comment, which says the same and instructs that this stated reason must not be quoted. |
...
| DEF-16 specifically | **PIN — do not clamp, but NOT by a golden (§10.9)** | Re-affirming #15's decision. `Math.max(0, delay2)` is a behaviour change — but an **unobservable** one: the clamping port was built and passed all five goldens byte-identically, so this row's pin has to be an L1 test (#28), not a scenario. The instantaneous traversal is part of the frozen baseline; a port must be *checked* against the measured Cybele timer semantics, not assumed to match. JADE's `WakerBehaviour` also fires at once for a past deadline — that coincidence is exactly what a golden should pin rather than a port assume. |
...
>
>   **Outcome ([#41](https://github.com/bedaHovorka/OpenCybele1/issues/41)): met, and by a route
>   this bullet did not anticipate — the premise "the bounded wait it will necessarily use" turned
>   out to be false.** The port has no bounded wait at all: `Planning` replaced `latch.await()` with
>   a non-blocking tally (`VoteRound`) plus a template gate, and `Station` replaced `wait()` with a
>   continuation queue. The requirement's *spirit* is honoured by two `TickerBehaviour`
>   watchdogs — `Planning.Watchdog` names the train, the silent voters and the elapsed time on
>   stderr where `ErrorScanner` reads; `Station.Watchdog` names a still-parked `PATH_FIND`
>   continuation — and **neither resumes anything**: no round is closed, no envelope is synthesised
>   from partial votes, no gate is released, and the leak is preserved. That is strictly stronger
>   than the bullet asked for, because there is no fall-through branch left to get wrong.
...
at a station**, an **NPE from `OueueItem.diff`**, a **`ConcurrentModificationException` from `TableModel.update`** (§4.5: it is thrown on the *agent* thread that just mutated the map, not on the EDT — this paragraph predates §4.5 and said "on the EDT"),
